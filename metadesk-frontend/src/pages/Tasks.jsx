...
import { Plus, Search, Clock, CheckSquare, Trash2 } from 'lucide-react'
...
export default function Tasks() {
  ...
  const load=()=>{const p=new URLSearchParams();if(tab!=='all')p.set('status',tab);if(search)p.set('search',search);api.get('/tasks?'+p).then(r=>setTasks(r.data.tasks||[])).catch(console.error).finally(()=>setLoading(false))}
  ...
  const deleteTask=async()=>{if(!deleteTarget)return;setDeleting(true);try{await api.delete('/tasks/'+deleteTarget._id);setTasks(p=>p.filter(t=>t._id!==deleteTarget._id));setDeleteTarget(null);toast.success('Task deleted')}catch(err){toast.error(err.response?.data?.message||'Failed to delete task')}finally{setDeleting(false)}}
...
export function CreateTaskModal({onClose,onCreated,initialProjectId='',canAssignOverride=false,assignableUsers=null}){
  ...
  const{user}=useAuth()
  const hasGlobalAssignAccess=can(user,'assignTasks')
  const canAssignTasks=hasGlobalAssignAccess||canAssignOverride
  const assigneeUsers=hasGlobalAssignAccess?users:(assignableUsers||users)
  const set=f=>e=>setForm(p=>({...p,[f]:e.target.value}))
  useEffect(()=>{Promise.all([api.get('/projects'),canAssignTasks?api.get('/users'):Promise.resolve({data:{users:[]}})]).then(([projectRes,userRes])=>{setProjects(projectRes.data.projects||[]);setUsers(userRes.data.users||[])}).catch(()=>{})},[canAssignTasks])
  const toggleAssignee=id=>setSelectedAssignees(prev=>prev.includes(id)?prev.filter(userId=>userId!==id):[...prev,id])
  const submit=async e=>{e.preventDefault();if(!form.title.trim())return toast.error('Title required');setLoading(true);try{await api.post('/tasks',{...form,project:project||null,assignedTo:canAssignTasks?selectedAssignees:[]});toast.success('Task created!');onCreated()}catch(err){toast.error(err.response?.data?.message||'Failed')}finally{setLoading(false)}}
  return(<Modal open onClose={onClose} title="New Task" width={560}><form onSubmit={submit} style={{padding:'20px 24px 24px',display:'flex',flexDirection:'column',gap:14}}><Input label="Title *" value={form.title} onChange={set('title')} placeholder="e.g. Design login page"/><div style={{display:'flex',flexDirection:'column',gap:5}}><label style={{fontSize:13,fontWeight:600,color:'#1A4A8A'}}>Description</label><textarea value={form.description} onChange={set('description')} placeholder="Details..." rows={3} style={{padding:'9px 12px',borderRadius:10,border:'1.5px solid #DBEAFE',fontSize:13,fontFamily:'inherit',outline:'none',resize:'vertical'}}/></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}><Select label="Priority" value={form.priority} onChange={set('priority')}>{['low','medium','high','critical'].map(v=><option key={v} value={v}>{v.charAt(0).toUpperCase()+v.slice(1)}</option>)}</Select><Input label="Due Date" type="date" value={form.dueDate} onChange={set('dueDate')}/></div><Select label="Project (optional)" value={project} onChange={e=>setProject(e.target.value)}><option value="">Standalone task</option>{projects.map(p=><option key={p._id} value={p._id}>{p.title}</option>)}</Select>{canAssignTasks&&<div><label style={{fontSize:13,fontWeight:600,color:'#1A4A8A',display:'block',marginBottom:8}}>Assignees</label><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(190px,1fr))',gap:8,maxHeight:170,overflowY:'auto',paddingRight:2}}>{assigneeUsers.map(member=>{const selected=selectedAssignees.includes(member._id);return(<button type="button" key={member._id} onClick={()=>toggleAssignee(member._id)} style={{display:'flex',alignItems:'center',gap:8,border:`1.5px solid ${selected?'#2F85C8':'#DBEAFE'}`,background:selected?'#EFF6FF':'#fff',borderRadius:10,padding:9,cursor:'pointer',textAlign:'left'}}><span style={{width:18,height:18,borderRadius:6,border:`1.5px solid ${selected?'#2F85C8':'#BAD6F4'}`,background:selected?'#2F85C8':'#fff',color:'#fff',display:'inline-flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:900,flexShrink:0}}>{selected?'x':''}</span><span style={{minWidth:0}}><span style={{display:'block',fontSize:12.5,fontWeight:700,color:'#0F1E3D',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{member.name}</span><span style={{display:'block',fontSize:11,color:'#667085',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{member.designation||member.email}</span></span></button>)})}</div></div>}<div style={{display:'flex',gap:10,justifyContent:'flex-end'}}><button type="button" onClick={onClose} style={{padding:'9px 20px',borderRadius:10,border:'1.5px solid #DBEAFE',background:'#fff',color:'#1A4A8A',fontSize:13,fontWeight:600,cursor:'pointer'}}>Cancel</button><button type="submit" disabled={loading} className="btn-primary">{loading?'Creating...':'Create Task'}</button></div></form></Modal>)
}
